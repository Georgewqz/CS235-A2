...
import movie_web_app.home.services as services
...
def register():
    ...
    return render_template(
        ...
        handler_url=url_for('home_bp.register'),
...
def home():
    ...
    ids = services.get_movie_by_id(repo.repo_instance)

    movies = services.get_movies(ids[cursor: cursor + 1], repo.repo_instance)

    prev_movie = None
    next_movie = None
    first_movie = url_for('home_bp.home')
    last_movie = url_for('home_bp.home', cursor=len(ids) - 1)
    if cursor != 0:
        prev_movie = url_for('home_bp.home', cursor=cursor - 1)
    if cursor != len(ids)-1:
        next_movie = url_for('home_bp.home', cursor=cursor + 1)
